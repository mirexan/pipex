...
static	void	waiting_for_children(t_args *args)
{
	...
	while (i < args->cmd_count)
	{
		...
		if (i == args->cmd_count - 1 && WIFEXITED(status))//esperamos
			exit_code = WEXITSTATUS(status);//el codigo de salida del ultimo comando
		i++;
	}
	ft_cleanup(args);
	exit(exit_code);
...
static void	manage_pipes(t_args *args, char **envp, int i)
{
	if (args->cmd_count == 1)
	{
